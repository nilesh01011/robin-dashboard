...
function UpcomingAccordion({ title, description, id, theme }) {
    ...
    return (
        <>
            {
                id && <div onClick={() => setShow(!show)} className={`items w-full overflow-hidden ${show ? 'h-max' : 'h-[34px]'} py-[10px] ${id === 3 && 'pb-0'}`}>
                    <div className={`w-[85%] flex items-center justify-between cursor-pointer`}>
                        <span className={`${theme === 'dark' ? 'text-white' : 'text-dark'} text-[14px] select-none`}>{title}</span>
